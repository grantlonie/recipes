...
def test_ref_helpers_recognize_urls_and_file_paths():
    ...
    assert not is_ref_url("sources/chili/image.jpg")
    assert is_ref_file("sources/chili/image.jpg")
...
def test_metadata_helpers_extract_flat_refs():
    ...
    assert metadata_image_file(metadata) == "sources/chili/image.jpg"


def test_resolve_image_url_maps_file_paths_to_api_route():
    metadata = {"image": "sources/chili/image.jpg"}
    assert (
...
def test_validate_ref_value_rejects_unknown_formats():
    ...
    assert validate_ref_value("sources/chili/source.pdf")
